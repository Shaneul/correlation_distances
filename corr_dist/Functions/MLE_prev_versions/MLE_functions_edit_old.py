...
def MLE(X:np.ndarray, k_min:int = 1, vt:int = 3, IC:str = 'both'):
    ...
    while np.std(votes[-vt:]) >= 0.1:   
        ...
        votes.append(np.argmax(weights))
    ...
    Final_dist = [k_min-vt, Distributions[np.argmax(weights)],Results[Distributions[np.argmax(weights)]][k_min-vt], Delta, weights]
    return Final_dist
...
def plotting(N, Input, fit, Name, result, emp, dist, save=False):
     plt.step(N, emp, '+', ms = 4, color = 'k', label = 'Actual')
     plt.plot(Input, fit, label = result[1])
     plt.xscale('log')
     plt.yscale('log')
     plt.xlabel('$k$', color = 'k', fontsize = 16)
     plt.ylim(ymin=0.5*np.min(emp))
     if dist == 'PDF':
         plt.ylabel(r'$p_k$', fontsize = 16)
     if dist == 'CCDF':
         plt.ylabel(r'$P_k$', fontsize = 16)
     plt.title(Name + ' ' + dist + r'$ k_{\rm min} =$'  + str(result[0]), fontsize=16)
     plt.legend()
     plt.savefig("Plots\\MLE_plots\\" + Name + "no split", dpi=300)
     plt.show()

def fit(Name, G, k_min:int=1, plot_type='auto',save=False):
    """
    Parameters
    ----------
    Name : String
    ...
    if plot_type == 'both' or plot_type == 'pdf':
        ...
        plotting(N,Input, pdf, Name, result, p, 'PDF', save)
    if plot_type == 'both' or plot_type == 'ccdf':
        ccdf = CCDF(result, Input, N, P)
        plotting(N, Input, ccdf, Name, result, P, 'CCDF', save)
    print('For k greater than ',k_min, ' the degree distribution follows a', result[1], 'distribution with parameters', np.round(result[2][0],2))
    return result
