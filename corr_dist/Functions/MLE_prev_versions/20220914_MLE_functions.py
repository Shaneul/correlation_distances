...
def bootstrap(G_list, result): 
    ...
    parameters : list of list
        list of lists of tuples. Each tuple is the parameters of a distribution fit to 
        bootstrapped sample
...
def MLE(X:np.ndarray, k_min:int = 1, vt:int = 3, IC:str = 'both'):
    """
    ...
    IC : str, optional, default = 'both'
        Can be 'AIC', 'BIC', or 'both'. Which information criteria to use.
    Returns
    -------
    Final_dist : list
        ...
        if IC == 'both':
            if Distributions[np.argmin(AICs)] == Distributions[np.argmin(BICs)]:
                ...
                votes += [10] #if BIC and AIC do not choose the same distribution, 
                          #move to next k_min
        if IC == 'AIC':
            votes.append(np.argmin(AICs).astype(np.int32))
    ...
    Final_dist = [k_min-vt, Distributions[np.argmin(AICs)],Results[Distributions[np.argmin(AICs)]][k_min-vt][0], Delta]
    return Final_dist
...
def fit(Name, G, k_min:int=1, plotting=True):
     """
     ...
     plotting : bool, optional
         Determines if the code will create plots of the distributions. Default is True
     ...
     if p_val < 0.1: #the distribution is not a good fit
         new_result = [MLE(X, result[0][0] + 1)] #we try MLE with slightly larger k_min 
         Y = [CCDF(new_result[0], N,N, P)]
         _, p_val = ks_2samp(P, Y[0])
         if p_val < 0.1: # the distribution is still not a good fit
     ...
     if plotting == True:    
         ...
         for i in range(len(Y)):
             plt.plot(Input[i], Y[i],label = result[i][1])
         plt.xscale('log')
         plt.yscale('log')
         plt.title(Name)
         plt.legend()#
     ...
     rows = []    
     if len(result) == 1:
         print('The degree distribution follows a', result[0][1], 'distribution with parameters', np.round(result[0][2],2))
     if len(result) == 2:
         print(Name,':For k less than', result[1][0], 'the degree distribution follows a', result[0][1],'\
               distribution with parameters', np.round(result[0][2]),'.\n For k greater than', result[1][0], 'the degree\
               distrubtion follows a ', result[1][1], 'distribution with parameters', np.round(result[1][2]))
     for i in result:
         params = bootstrap(X, i)
         rows.append(summary_stats(Name, i, params))
     return result, Input, Y, rows #bootstrapping commented out while testing kmin+1
