...
def plotting(N, Input, fit, Name, result, emp):
     plt.step(N, emp, '+', ms = 4, color = 'k', label = 'Actual')
     plt.plot(Input, fit, label = result[1])
     plt.xscale('log')
     plt.yscale('log')
     plt.ylim(ymin=0.5*np.min(emp))
     plt.title(Name + ' kmin ='  + str(result[0]))
     plt.legend()
     plt.show()

def fit(Name, G, k_min:int=1, plot_type='auto'):
    """
    Parameters
    ----------
    Name : String
    ...
    if plot_type == 'both' or plot_type == 'pdf':
        ...
        plotting(N,Input, pdf, Name, result, p)
    if plot_type == 'both' or plot_type == 'ccdf':
        ccdf = CCDF(result, Input, N, P)
        plotting(N, Input, ccdf, Name, result, P)
