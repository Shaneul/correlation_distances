...
def compound_poisson(x, x0):
    
    l_max = []
    for i in range(len(x0)):
        l = minimize(cmpnd_l,x0[:i+1], method='SLSQP')['fun']
        if np.isnan(l):
            l = np.inf
    l_max.append(AIC(l,x,len(x0[:2*(i+1)])))
    
    n_c = np.argmin(l_max) + 1
    
    opt_cmp = minimize(cmpnd_l, x0[:n_c], method='SLSQP')
    return opt_cmp
...
def MLE(X:np.ndarray, k_min:int = 1, vt:int = 3, IC:str = 'AIC'):
    ...
    while np.std(votes[-vt:]) >= 0.1: # while the last X votes have not been the same
                                      # where X is vt.
        ...
        opt_cmp = compound_poisson(x, [k_min*2, x.mean(), x.max()])
...
def plotting(N, Input, fit, result, emp, dist, Name = '', save=False, saveloc=''):
    ...
    if save == True:
        if saveloc == '':
            raise ValueError('save is True but no save location provided. Please\
                             enter a folder path to save plot to')
...
def fit(Name, G, k_min:int=1, plot_type='auto',save=False, saveloc='', IC='AIC'):
    """
    Parameters
    ----------
    Name : String
    ...
    print('For k greater than or equal to',k_min, ' the degree distribution follows a', 
